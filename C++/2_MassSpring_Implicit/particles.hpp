...
//applys gravity force on all particles
...
public:
	...
	float amount;
	ParticleSystem *ps;
	Gravity(ParticleSystem* ps, float amount = -9.8f): ps(ps), amount(amount) {}
	void addForces(VectorXd &f);
	void addJacobians(MatrixXd &Jx, MatrixXd &Jv);
	void draw();
};

//applys ground force on all particles
class GroundForce: public Force {
public:
	
	ParticleSystem *ps;
	float yLow;
	GroundForce(ParticleSystem* ps, float yLow=0) : ps(ps), yLow(yLow) {}
	void addForces(VectorXd &f);
	void addJacobians(MatrixXd &Jx, MatrixXd &Jv);
	void draw();
};
//limits particle system in the playarea
class BoundsForce : public Force {
public:
	
	ParticleSystem *ps;
	float xMin, yMin, xMax, yMax;
	BoundsForce(ParticleSystem* ps, float xMin= 0, float yMin=0, float xMax=1, float yMax=1) : ps(ps), xMin(xMin), xMax(xMax), yMin(yMin), yMax(yMax) {}
	void addForces(VectorXd &f);
	void addJacobians(MatrixXd &Jx, MatrixXd &Jv);
	void draw();
};
